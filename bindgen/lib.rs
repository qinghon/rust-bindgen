...
pub struct Bindings {
    options: BindgenOptions,
    module: ModulesTokenStream,
}
/// Generated Rust bindings with file location.
#[derive(Debug)]
pub struct ModulesTokenStream {
    pub(crate) modules: Vec<proc_macro2::TokenStream>,
    /// (filename, <`modules_index`>)
    pub(crate) files: Vec<(String, usize)>,
}
...
impl Bindings {
    ...
    fn format_tokens_to_string(
        ...
        match self.format_tokens(tokens) {
            Ok(formatted_bindings) => formatted_bindings,
            Err(err) => {
                eprintln!(
    ...
    pub fn append_module(
        ...
        writer.write_all(self.format_tokens_to_string(module).as_bytes())
    }
    ...
    pub fn write<'a>(&self, mut writer: Box<dyn Write + 'a>) -> io::Result<()> {
        ...
        for module in &self.module.modules {
            self.append_module(writer.as_mut(), module)?;
        }
    ...
    fn write_by_mod(
        ...
        self.append_module(writer, module)
    }
    /// split module by file from callback to string
    pub fn token_stream_by_header<F>(&self, mut header_map_fn: F)
    where
        F: FnMut(&str, &proc_macro2::TokenStream),
    {
        header_map_fn("", &self.module.modules[0]);
        for (f, index) in &self.module.files {
            header_map_fn(f.as_str(), &self.module.modules[*index]);
        }
    }
    /// split module by file from callback to file
    pub fn write_to_file_by_header<P: AsRef<Path>, F>(
        &self,
        header_map_fn: F,
    ) -> io::Result<()>
    where
        F: Fn(&str) -> Option<P>,
    {
        let builtin = header_map_fn("");
        if let Some(path) = builtin {
            let mut file = OpenOptions::new()
                .write(true)
                .truncate(true)
                .create(true)
                .open(path.as_ref())?;
            self.write_by_mod(&mut file, &self.module.modules[0])?;
        }
        for (f, index) in &self.module.files {
            if let Some(path) = header_map_fn(f.as_str()) {
                let mut file = OpenOptions::new()
                    .write(true)
                    .truncate(true)
                    .create(true)
                    .open(path.as_ref())?;
                self.write_by_mod(&mut file, &self.module.modules[*index])?;
            }
        }
